...
begin
  ...
  if isMacroQuoted(s) then
    result := copy(s, length(MARKER_QUOTE) + 1, length(s) - length(MARKER_QUOTE)
      - length(MARKER_UNQUOTE));
end; // macroDequote
...
var
  ...
  begin
    result := '<div class=macroerror>macro error: ' + name +
      nonEmptyConcat('<br>', msg) + '</div>'
  end;

  procedure deprecatedMacro(what: string = ''; instead: string = '');
  begin
    mainfrm.add2log('WARNING, deprecated macro: ' + first(what, name) +
      nonEmptyConcat(' - Use instead: ', instead), NIL, clRed)
  end;
  ...
  function parEx(idx: integer; name: string = ''; doTrim: boolean = true)
    : string; overload;
  var
    i: integer;
  begin
    ...
    if (idx < 0) // no numeric index accept
      ...
      or (name > '') and (pars.names[idx] > '') and
      not anycharIn(' '#13#10, pars.names[idx])
    // this numerical index was already taken by a valid mnemonic name
    then
  ...
  function par(idx: integer; name: string = ''; doTrim: boolean = true)
    : string; overload;
  begin
    try
  ...
  function par(name: string = ''; doTrim: boolean = true; defval: string = '')
    : string; overload;
  begin
    try
      result := parEx(-1, name, doTrim)
...
// Reason for having this instead of using in place a simple "result:=if_(cond, ' ')" is to evidence our purpose. It's not faster or cleaner, it's more semantic.
  ...
  begin
    result := getVarSpace(varname).values[varname]
  end;
...
// if par with name exists, then it's a var name, otherwise it's a constant value at specified index
  ...
  function setVar(varname, value: string;
    space: THashedStringList = NIL): boolean;
  var
    o: Tobject;
    i: integer;
  begin
    ...
    if i < 0 then
      if value = '' then
        exit // all is good the way it is
      else
        i := space.add(varname + '=' + value)
    else if value > '' then
    // in case of empty value, there's no need to assign, because we are going to delete it (after we cleared the bound object)
  ...
  function pos_(caseSensitive: boolean; ss, s: string;
  ...
  begin
    ...
    begin
      result := par(i);
      if isAnd xor isTrue(result) then
        exit;
    end;
  end; // allLogic
  ...
  begin
    ...
    try
      ...
      if s = 'none' then
        ...
        what2inc := 1; // by default we include only the first delimiter
    end;
    ...
    if pars[0] = '' then
      ...
      i := pos_(caseSens, pars[0], s);
    // we don' trim this, so you can use blank-space as delimiter
    if i = 0 then
      exit;
  ...
  begin
    what := par(0);
    sep := first(pars[1], ' ');
    // we don' trim this, so you can use blank-space as separator
    i := 2;
  ...
  begin
    ...
    begin
      v := parF(i);
      if (v < r) and min or (v > r) and not min then
        r := v;
    end;
    result := floatToStr(r);
  end; // minOrMax
  ...
  begin
    ...
    try
      ...
      // if no var is specified, it will break here, and result will have the value
      result := '';
    except
    end;
  end; // urlVar
  ...
  begin
    result := '';
    try
      cs := [#0 .. #255] - strToCharset(parEx('only'));
    except
      cs := ['a' .. 'z', 'A' .. 'Z', '0' .. '9', ',', '/', '#', '&', '?', ':',
        '$', '@', '=', '+'] - strToCharset(par('add')) +
        strToCharset(par('not'));
    end;
  ...
  begin
    ...
    if assigned(old) then
      if not old.isRoot() and (not parExist(['overwrite']) or
        isTrue(par('overwrite'))) then
        try
          old.node.delete()
        except
  ...
  var
    ...
    begin
      result := onlyExistentAccounts
        (split(';', parEx(prefix + FILEACTION2STR[act])));
  ...
  begin
    ...
    try
      f.setDynamicComment(macroDequote(parEx('comment')))
    except
    end;
    try
      f.resource := parEx('resource')
    except
    end;
    try
      f.diffTpl := parEx('diff template')
    except
    end;
    try
      f.filesFilter := parEx('files filter')
    except
    end;
    try
      f.foldersFilter := parEx('folders filter')
    except
    end;

    // following commands make no sense on temporary items
    if freeIfTemp(f) then
      exit;
    ...
    if f.isFolder() then
      try
        f.dontCountAsDownloadMask := parEx('not as download')
      except
      end
    else
    ...
    begin
      ...
      try
        addUniqueArray(f.accounts[act], get('add '))
      except
      end;
      try
        removeArray(f.accounts[act], get('remove '))
      except
      end;
    end;
    VFSmodified := true;
  ...
  begin
    ...
    spaceIf(assigned(f));
    // so you can know if something really has been deleted
    if f = NIL then
      exit; // doesn't exist
  ...
  begin
    ...
    try
      ...
      if w = 'exists' then
        ...
        result := f.resource
      else if getAttr('hide', FA_HIDDEN) or getAttr('hide tree', FA_HIDDENTREE)
  ...
  begin
    ...
    try
      ...
      timeout := parF('timeout', 1) / SECONDS;
      // stay safe: 1 second timeout by default
      start := now();
      repeat
        if (timeout > 0) and (now() - start > timeout) then
          break;
  ...
  function uri2diskMaybeFolder(s: string): string;
  // like uri2diskMaybe, but limited to the path, excluding the filename
  var
    path: string;
  begin
  ...
  begin
    ...
    begin
      ...
      if i < 0 then
        exit; // this var doesn't exit. won't write.
      encode := FALSE;
  ...
  procedure dialog();
  const
    STR2CODE: array [1 .. 7] of string = ('okcancel=1', 'yesno=4',
      'yesnocancel=3', 'error=16', 'question=32', 'warning=48',
      'information=64');
  var
    i, j, code: integer;
  ...
  begin
    ...
    case msgDlg(p, code, par(2)) of
      MRYES, MROK:
        result := if_(buttons, '1');
        // if only OK button is available, then return nothing
      MRCANCEL:
        result := if_(code and MB_YESNOCANCEL = MB_YESNOCANCEL, 'cancel');
        // for the YESNOCANCEL, we return cancel to allow to tell NO from CANCEL
    else
      result := '';
    end;
  end; // dialog
  ...
  begin
    ...
    try
      a.redir := parEx('redirect')
    except
    end;
    try
      a.noLimits := isTrue(parEx('no limits'))
    except
    end;
    try
      a.enabled := isTrue(parEx('enabled'))
    except
    end;
    try
      a.group := isTrue(parEx('is group'))
    except
    end;
    try
      a.link := split(';', parEx('member of'))
    except
    end;
    try
      addArray(a.link, split(';', parEx('add member of')))
    except
    end;
    try
      removeArray(a.link, split(';', parEx('remove member of')))
    except
    end;
    try
      a.notes := parEx('notes')
    ...
    try
      a.notes := setKeyInString(a.notes, parEx('notes key'))
    except
    end;
  end; // setAccount
  ...
  begin
    ...
    if s = 'redirect' then
      ...
      result := '1';
    try
      result := getKeyFromString(a.notes, parEx('notes key'))
    except
    end;
  end; // getterAccount
  ...
  begin
    ...
    for i := 0 to srv.conns.count - 1 do
      with conn2data(i) do
        if addressmatch(ipmask, address) and
          ((portmask = '') or filematch(portmask, conn.port)) then
          conn.disconnect();
    result := '';
  end; // disconnect
  ...
  begin
    ...
    if fileOrDirExists(s) then
      s := quoteIfAnyChar(' ', s)
    else if unnamedPars < 2 then
      s := '';
    if parExist(['out']) or parExist(['timeout']) or parExist(['exit code'])
    then
      try
        spaceIf(captureExec(macroDequote(p) + nonEmptyConcat(' ', s), s, code,
          parF('timeout', 2)));
        try
          setVar(parEx('exit code'), intToStr(code))
        except
        end;
        setVar(parEx('out'), s);
      except
  ...
  begin
    trueIf(assigned(f) and f.hasRecursive(FA_ARCHIVABLE) or (f = NIL) and
      md.archiveAvailable)
  end;
  ...
  begin
    ...
    result := '';
    try
      md.cd.conn.setCookie(p, parEx('value'), getPairs());
    except
      result := noMacrosAllowed(md.cd.conn.getCookie(p))
    end; // there was no "value" to set, so just read
  end; // cookie
  ...
  procedure handleSymbol();
    ...
    begin
      if md.f = NIL then
        exit;
      if (md.f.icon >= 0) or (mainfrm.useSystemIconsChk.checked and
        md.f.isFile()) then
  ...
  begin
    ...
    i := length(md.table) - 2;
    if odd(i) then
      dec(i); // ensure the table has a legal length
    ...
    result := name;
    // by default, an unrecognized symbol remains the same (just as the song)
    ...
    if ansiStartsText('%sym-', name) then // legacy: surpassed by {.section.}
      ...
      if assigned(md.f) and assigned(md.tpl) and md.f.hasRecursive(FA_ARCHIVABLE)
      then
    ...
    begin
      ...
      if name = '%host%' then
        ...
        result := stripChars(copy(md.cd.conn.getHeader('Accept-Language'), 1,
          2), ['a' .. 'z', 'A' .. 'Z'], true)
    ...
    if assigned(md.folder) then
      ...
      begin
        ...
        result := smartSize(diskSpaceAt(md.folder.resource) -
          minDiskSpace * MEGA)
        ...
        with md.folder do
          result := optUTF8(md.tpl, if_(isRoot(), '/',
            getFolder() + name + '/'));
    ...
    if assigned(md.f) then
      ...
      begin
        ...
        result := macroQuote(optUTF8(md.tpl, copy(extractFileExt(md.f.name),
          2, MAXINT)))
        ...
        result := if_(md.f.mtime = 0, 'error',
          optUTF8(md.tpl, dateTimeToStr(md.f.mtime)))
    ...
    if ansiContainsText(name, 'folder') and not ansiContainsText(name, 'comment')
    then
      result := macroQuote(result);
  end; // handleSymbol
...
begin
  try
    ...
    try
      ...
      name := trim(pars[0]);
      pars.delete(0);
      // this operation is done with a memory move over pointers. Having few parameters normally, it's fast. We may eventually avoid this deletion and consider parameters starting from 1.
      if name = '' then
        exit;
      ...
      if name[1] = '!' then
        ...
        try
          result := first([fromTable('special:strings', p),
            md.tpl.getStrByID(p), par(0), p])
        except
        end;
      ...
      infixOperators(['>=', '<=', '<>', '!=', '=', '>', '<']);
      // the order is important, because >= would be confused with =
      ...
      if name = 'count' then
        if satisfied(md.cd) then
          result := intToStr(md.cd.tplCounters.incInt(p) - 1);
      // it can work even with no parameters
      ...
      begin
        ...
        result := optUTF8(md.tpl, result);
        // we may have special chars from datetimeToString()
      end;
      ...
      if name = 'current downloads' then
        result := intToStr(countDownloads(par('ip'), par('user'),
          if_(sameText(par('file'), 'this'), md.f) as Tfile));
      ...
      if name = 'vfs select' then
        if pars.count = 0 then
          ...
          try
            s := parEx('path');
            spaceIf(FALSE);
            mainfrm.filesBox.selected := mainfrm.findFilebyURL(s, NIL,
              FALSE).node;
            spaceIf(true);
          except
          end;
      ...
      if pars.count < 1 then
        exit; // from here, only macros with parameters
      ...
      begin
        if reMatch(p, '^https?://', 'i!') > 0 then
          ...
          i64 := sizeOfFile(uri2diskMaybe(p));
        result := intToStr(max(0, i64));
        // we return 0 instead of -1 for non-existent files, t
      end;
      ...
      if name = 'header' then
        if satisfied(md.cd) then
          try
            result := noMacrosAllowed(md.cd.conn.getHeader(p))
          except
          end;
      ...
      if name = 'get' then
        try
          if p = 'can recur' then
            trueIf(mainfrm.recursiveListingChk.checked)
            ...
            trueIf(md.f.isNew())
          else if p = 'agent' then
            result := md.cd.agent
          else if p = 'tpl file' then
            result := tplFilename
          else if p = 'protocolon' then
            result := protoColon()
          else if p = 'speed limit' then
            result := intToStr(round(speedLimit))
          ...
          begin
            ...
            result := join(';', getAccountList(stringExists(par(1),
              ['', 'users']), stringExists(par(1), ['', 'groups'])));
        except
          unsatisfied()
        end;
      ...
      if name = 'vfs to disk' then
        if isAbsolutePath(p) then
          ...
          result := expandFileName
            (includeTrailingPathDelimiter(md.folder.resource) + p)
        else
      ...
      begin
        result := '';
        if satisfied(md.cd) then
          md.cd.conn.reply.contentType := p;
      end;
      ...
      if name = 'notify' then
      begin
        tray.balloon(p, parF('timeout', 3), stringTotrayMessageType(par('type')
          ), par('title'));
        result := '';
      end;
      ...
      if pars.count < 2 then
        exit; // from here, only macros with at least 2 parameters
      ...
      if name = 'rename' then
      begin
        spaceIf(not isExtension(par(1), '.lnk') and
          // security matters (by mars)
          renameFile(uri2diskMaybe(p), uri2diskMaybeFolder(par(1))));
        if (result > '') and not stopOnMacroRename then
        // should we stop recursion?
          try
            ...
            runEventScript('on macro rename',
              toSA(['%old-name%', p, '%new-name%', par(1)]), md.cd);
          finally
      ...
      if name = 'pos' then
        result := intToStr(pos_(isTrue(par('case')), parVar('what', 0),
          parVar('var', 1), strToIntDef(par('from'), 1)));
      ...
      if stringExists(name, ['if', 'if not']) then
        if isFalse(p) xor (length(name) > 2) then
      ...
      if pars.count < 3 then
        exit; // from here, only macros with at least 3 parameters
...
begin
  ...
  try
    md.aliases := defaultAlias;
    // we don't even create a new object if not necessary
    if assigned(md.tpl) then
    begin
