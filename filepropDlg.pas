...
type
  ...
  public
    ...
    savePerm: array [TfileAction] of boolean;
    // should we apply/save permissions for this TfileAction ?
...
procedure TfilepropFrm.accountsBoxChange(Sender: TObject; Item: TListItem;
  Change: TItemChange);
begin
  if (Change = ctState) and (Item.caption > '') and
    (stringExists(Item.caption, users[currAction]) <> Item.checked) then
  begin
...
procedure TfilepropFrm.accountsBoxGetImageIndex(Sender: TObject;
  Item: TListItem);
begin
  Item.ImageIndex := accountIcon(Item.data)
end;
...
begin
  if pages.focused then
    if Key in ['1' .. '9'] then
      try
        pages.tabIndex := ord(Key) - ord('0') - 1
      except
      end;
end;
...
var
  ...
  begin
    ...
    if i = 0 then
      cb.state := should
    else if (cb.state <> cbGrayed) and (cb.state <> should) then
      cb.state := cbGrayed;
  end; // setFlag
  ...
  begin
    ...
    if (pos(#0 + v + #0, box.hint) > 0) or (box.hint = #0) and (v = box.text)
    then
    ...
    try
      (box as TLabeledEdit).COLOR := COLOR except
    end;
    try
      (box as TMemo).COLOR := COLOR except
    end;
  end; // setText
...
begin
  ...
  i := if_(mainFrm.filesBox.SelectionCount > 1, -1,
    selectedFile.getIconForTreeview());
  ...
  begin
    ...
    begin
      ...
      else if iconBox.itemIndex <> f.icon + iconOfs then
        iconBox.itemIndex := -1;
    end;
  ...
  if easyMode then
    onlyForExperts([browsableChk, commentTab, realmBox, dontconsiderChk,
      maskTab, dontlogChk, hideextChk]);
...
var
  ...
  begin
    if (cb.state = cbGrayed) or not cb.enabled or not cb.Visible then
      exit;
...
begin
  ...
  begin
    ...
    for act := low(act) to high(act) do
      if savePerm[act] and ((act <> FA_UPLOAD) or f.isRealFolder()) and
        ((act <> FA_DELETE) or f.isFolder()) then
      begin

        // The following is because we monitor every upload path
        if (act = FA_UPLOAD) and ((f.accounts[act] = NIL) <> (users[act] = NIL))
...
begin
  ...
  if countSubstr(#0, box.hint) < 2 then
    exit;

  s := box.hint;
  h := chop(#0, s);
  if not chooseValue(s) then
    exit;
  box.text := s;
  box.hint := h;
  // these properties are unhappily kept unaccessible through TcustomEdit interface
  try
    (box as TLabeledEdit).COLOR := clWindow except
  end;
  try
    (box as TMemo).COLOR := clWindow except
  end;
end;
